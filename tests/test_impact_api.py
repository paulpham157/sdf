...
def test_scorecard_reflects_a_real_task_attempt_evidence_chain(tmp_path):
    # /scorecard aggregates over every Task ever created against the shared
    ...
    before = {item["name"]: item for item in client.get("/scorecard").json()["metrics"]}
    ...
    after = {item["name"]: item for item in client.get("/scorecard").json()["metrics"]}
